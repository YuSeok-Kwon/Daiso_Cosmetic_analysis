...
SAMPLING_CONFIG = {
    "target_size": 20000,
    "category_1_column": "category_1",
    "category_2_column": "category_2",
    "category_1_min_floor": 600,
    "category_2_min_floor": 200,
    "exclude_categories": [],
...
|------|-----------|---------|
| 복원추출 | O (부족 시) | X |
| Sentiment 강제 | 소분류 단위 | 전체 레벨 |
| 부족 클래스 | 복원추출로 채움 | 학습 시 보정 |
| 중복 샘플 | 발생 가능 | 없음 |
| 자연 분포 | 훼손됨 | 유지됨 |
...
| 메서드 | 설명 |
|--------|------|
| `_assign_sentiment_group()` | 평점 기반 sentiment 그룹 할당 |
| `_calculate_quotas_with_min_floor()` | 최소 보장 + 비례 배분 쿼터 계산 |
| `sample()` | 4단계 층화 샘플링 수행 |
| `_balance_sentiment_at_total_level()` | 전체 레벨 sentiment 균형 조정 |
| `_validate_sample()` | 샘플링 결과 검증 |
