...
화장품 원료의 **영문명**, **CAS No** 등 상세 정보를 제공하는 API.
...
| 신청 페이지 | [공공데이터포털](https://www.data.go.kr/data/15111774/openapi.do) |
| 총 데이터 | 21,696건 |
...
`ingredients_master.csv`에 다음 컬럼 추가:
...
## 10. 화장품 사용제한 원료정보 API

### 10.1 개요

화장품에 사용이 **금지**되거나 **제한**된 원료 정보를 제공하는 API.
...
| 신청 페이지 | [공공데이터포털](https://www.data.go.kr/data/15111772/openapi.do) |
| 총 데이터 | 31,191건 (고유 원료: 6,924개) |

### 10.3 응답 컬럼

| 컬럼명 | 설명 |
|--------|------|
| REGULATE_TYPE | 규제 구분 (금지/한도/한도·금지) |
| INGR_STD_NAME | 표준명 |
| INGR_ENG_NAME | 영문명 |
| CAS_NO | CAS 번호 |
| COUNTRY_NAME | 배합제한 국가 |
| LIMIT_COND | 제한사항 상세 |
...
| 파일명 | 용도 | 건수 |
|--------|------|------|
| products_mfds.parquet | 기능성화장품 매칭 결과 | 186건 |
| products_mfds.xlsx | 위와 동일 (Excel) | 186건 |
| ingredients_master.csv | 성분 마스터 (보강) | 1,744건 |

### 11.2 ingredients_master.csv 최종 컬럼
